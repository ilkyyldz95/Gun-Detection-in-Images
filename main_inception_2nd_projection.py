...
if __name__ == '__main__':
    ...
    # I have gathered the test images from second part of the dataset.
    # Now find the unused images from the first half of the dataset
    # And add these images to test data.

    num_imag_pos = len(listdir(data_path_pos))/2
    num_imag_neg = len(listdir(data_path_neg))/2
    # List of all image names in the dataset
    all_images = listdir(data_path_pos)[:num_imag_pos] \
                 + listdir(data_path_neg)[:num_imag_neg]

    '''Initialize data'''
    print 'Starting k fold calculations...2nd projection test images from first projection\n'
    # Initialize K-fold operator
    kf = KFold(n_splits=K, shuffle=True, random_state=1) # Define the split - into K folds
    # train_index, test_index = list(kf.split(range(num_imag_pos+num_imag_neg)))[int(args.findex)]
    train_index, test_index = list(kf.split(all_images))[int(args.findex)]

    x_test2 = np.zeros((len(test_index),335,472,3), dtype=np.uint8)
    y_test2 = np.zeros(len(test_index), dtype=np.uint8)
    ...
    print 'x_test elements(should be around 40k):', len(x_test)
    print 'y_test elements(should be around 40k):', len(x_test)
